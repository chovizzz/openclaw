...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("marks a bootstrap file missing after transient read retries are exhausted", async () => {
    ...
    try {
      // The reader exhausts its retry budget (3 attempts) and returns an io
      // failure (not a throw), so the file surfaces as missing for this turn
  ...
  it("retries a transient open failure before dropping a bootstrap file", async () => {
    ...
    // openBoundaryFile reports a transient open failure as reason "io"; the
    // reader must retry it (re-open) rather than drop the file for the turn.
  ...
  it("does not retry a deterministic (non-transient) read failure", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");
    await writeWorkspaceFile({
      dir: tempDir,
      name: DEFAULT_AGENTS_FILENAME,
    ...
    // ENOENT is not in the transient-error set, so a single failure should be
    // final: no retry attempts and the file surfaces as missing immediately.
    let readFileSyncCalls = 0;
    const readSpy = vi.spyOn(syncFs, "readFileSync").mockImplementation((() => {
      readFileSyncCalls += 1;
