...
describeNonWin("exec script preflight", () => {
  ...
  it("does not trust a swapped script pathname between validation and read", async () => {
    await withTempDir("openclaw-exec-preflight-race-", async (rawParent) => {
      // Canonicalize: on macOS os.tmpdir() is /var/... while the safe-read path
      // resolves the root to /private/var/..., which would defeat the spies below.
      const parent = await fs.realpath(rawParent);
      const workdir = path.join(parent, "workdir");
  ...
  it("handles pre-open symlink swaps without surfacing preflight errors", async () => {
    await withTempDir("openclaw-exec-preflight-open-race-", async (rawParent) => {
      // Canonicalize: see the pathname-swap test above.
      const parent = await fs.realpath(rawParent);
      const workdir = path.join(parent, "workdir");
      const scriptPath = path.join(workdir, "script.js");
      const outsidePath = path.join(parent, "outside.js");
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("openclaw-exec-preflight-nonblock-", async (rawTmp) => {
      // Canonicalize: see the pathname-swap test above.
