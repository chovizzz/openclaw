...
// One watcher per unique watch target. Agent workspaces that include the same
// shared skill root (the global skills dir, the home skills dir, or a
// configured extra/plugin dir) subscribe to the same watcher instead of each
// opening its own, so open file descriptors scale with distinct targets rather
// than with agent count.
...
function createSkillsPathWatcher(watchPath: string, debounceMs: number): SkillsPathWatchState {
  const watcher = chokidar.watch(watchPath, {
    ignoreInitial: true,
    // Skill discovery reads root skills, direct child skills, and one grouped
    // skill level. Bound traversal so a target that resolves to a directory can
    // never walk an unrelated large tree.
    depth: 2,
    awaitWriteFinish: {
  ...
  const schedule = (changedPath?: string) => {
    ...
    state.timer = setTimeout(() => {
      ...
      // Fan the change out to every workspace subscribed to this target so a
