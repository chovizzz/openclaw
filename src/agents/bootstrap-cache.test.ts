...
describe("getOrLoadBootstrapFiles", () => {
  ...
  it("refreshes from disk on second call while preserving unchanged object identity", async () => {
    ...
    // Same content on disk (by value) -> callers keep the original cached array identity.
    expect(result).toBe(first);
    expect(result).not.toBe(refreshedFiles);
    expect(mockLoad()).toHaveBeenCalledTimes(2);
  });

  it("replaces cached result when workspace bootstrap contents change (reverse case)", async () => {
    ...
    expect(first).toBe(files);
    // Content actually changed -> the refreshed snapshot must be returned, not the stale one.
...
describe("clearBootstrapSnapshot", () => {
  ...
  it("does not affect other sessions", async () => {
    ...
    // sk2 should still preserve its cached snapshot identity after refresh (disk content
    // is unchanged across calls in this test's mock).
