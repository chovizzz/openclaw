        ...
        for await (const event of anthropicStream) {
          ...
          if (event.type === "content_block_delta") {
            ...
            if (!block && delta?.type === "text_delta" && typeof delta.text === "string") {
              // Some upstreams emit a text delta without the matching
              // content_block_start. Synthesize the block instead of discarding
              // the text, which would silently truncate the assistant reply.
