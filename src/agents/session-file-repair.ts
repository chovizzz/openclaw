 ...
 * JSONL corruption. They cannot be replayed to any provider — every provider
 * router branches on `message.role` — and preserving them through repair just
 * relocates the corruption from the original file into the post-repair file.
 * Treat them as malformed lines: drop during repair so the cleaned transcript
 * no longer carries them. The `.bak-*` backup keeps the original bytes.
 */
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (isStructurallyInvalidMessageEntry(entry)) {
        // Drop "null role" / missing-role message entries the same way we drop
        // unparseable JSONL: they cannot be replayed to any provider and
        // preserving them through repair just relocates the corruption into
        // the post-repair file.
        droppedLines += 1;
        continue;
      }
      entries.push(entry);
