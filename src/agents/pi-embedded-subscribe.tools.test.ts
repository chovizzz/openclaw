...
describe("sanitizeToolResult structural safety", () => {
  it("keeps repeated sibling references instead of collapsing them to [Circular]", () => {
    const shared = { host: "build-07", port: 8080 };
    const sanitized = sanitizeToolResult({ first: shared, second: shared }) as {
      first: Record<string, unknown>;
      second: Record<string, unknown>;
    };
    expect(sanitized.first).toEqual(shared);
    expect(sanitized.second).toEqual(shared);
    expect(JSON.stringify(sanitized)).not.toContain("[Circular]");
  });

  it("marks a genuine cycle without recursing forever", () => {
    const cyclic: Record<string, unknown> = { host: "build-07" };
    cyclic.self = cyclic;
    const sanitized = sanitizeToolResult(cyclic) as Record<string, unknown>;
    expect((sanitized.self as Record<string, unknown>).self).toBe("[Circular]");
    expect(sanitized.host).toBe("build-07");
  });

  it("redacts secrets embedded in object keys", () => {
    const sanitized = sanitizeToolResult({
      "https://example.test/cb?token=abcdef1234567890ghij": "ok",
    }) as Record<string, unknown>;
    const keys = Object.keys(sanitized);
    expect(keys[0]).not.toContain("abcdef1234567890ghij");
    expect(keys[0]).toContain("https://example.test/cb?token=");
    expect(sanitized[keys[0]]).toBe("ok");
  });
});
