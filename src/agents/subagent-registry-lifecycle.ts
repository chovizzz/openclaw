...
export function createSubagentRegistryLifecycleController(params: {
  ...
  const startSubagentAnnounceCleanupFlow = (runId: string, entry: SubagentRunRecord): boolean => {
    // A completion announce was already delivered for this exact run. Re-running
    // the announce flow (resume, descendant-settle wake, restart) would deliver
    // the same completion message to the parent a second time. Finish cleanup
