...
function openTaskRegistryDatabase(): TaskRegistryDatabase {
  ...
  const safety = resolveSqliteFilesystemSafety(resolveTaskRegistryDir(process.env));
  ...
  if (safety.status === "unsafe") {
    ...
    log.warn(
      "Task registry could not confirm its database filesystem is safe for WAL; keeping journal_mode=WAL. Verify the path is on a local disk, not a VM share or network mount.",
