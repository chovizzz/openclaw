...
//  3. fall back to the shared secret-pattern redaction for everything else,
//     which catches `KEY=VALUE` env-style assignments and raw token shapes
//     (sk-, ghp_, xox*, gsk_, AIza*, npm_, Telegram bot tokens, PEM blocks,
//     Bearer headers). Tools mode is forced: this is a persisted audit log,
//     not a `logging.redactSensitive` decision.
export function redactConfigAuditArgv(argv: readonly string[]): string[] {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // Forced tools-mode only. This runs *during* config load, and the
    // no-options variant of redactSensitiveText re-enters loadConfig() to read
    // `logging.redactPatterns`, which recurses back into this audit path.
    // Built-in patterns are always applied; user patterns cannot be honored here.
...
function resolveConfigAuditProcessInfo(
  ...
  if (processInfo) {
    // Caller-supplied argv is untrusted for this purpose too: redact it.
    return {
      ...processInfo,
