...
export function loadSessionStore(
  ...
  // An oversized store is loaded into memory on every read; prune it here so an
  // unattended gateway cannot OOM before the periodic maintenance pass runs.
  ...
  if (maintenance.mode === "enforce" && Object.keys(store).length > maintenance.maxEntries) {
    ...
    if (pruned > 0 || capped > 0) {
      // The on-disk serialization no longer matches the in-memory store.
