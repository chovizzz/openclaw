  ...
  async getBuffer(params: {
    ...
    if (entry.resolvedPath) {
      try {
        ...
        // A blocked/unreadable local path may still have a usable remote URL.
        // Anything else (including maxBytes) stays fail-closed.
        if (
    ...
    if (entry.resolvedPath) {
      if (params.maxBytes) {
        try {
          ...
          // Blocked/unreadable local paths clear resolvedPath and fall through to
          // the URL path below; other skip reasons stay fail-closed.
          if (
            !(err instanceof MediaUnderstandingSkipError) ||
            (err.reason !== "blocked" && err.reason !== "empty")
          ) {
