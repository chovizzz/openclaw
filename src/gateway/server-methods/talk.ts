...
function resolveTalkResponseFromConfig(params: {
  ...
  // provider plugins call strict secret helpers that throw on unresolved
  // wrappers, and this discovery path does not need the resolved value. The
  // response's apiKey is restored from source so the UI keeps the SecretRef
  // shape, and redaction strips the value when includeSecrets=false.
