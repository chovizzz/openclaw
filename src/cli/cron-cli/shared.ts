...
// Legacy/malformed persisted jobs can carry non-string cells; render them
// defensively instead of throwing while printing the list.
...
export function printCronList(jobs: CronJob[], runtime: RuntimeEnv = defaultRuntime) {
  ...
  for (const job of jobs) {
    ...
    const agentLabel = pad(truncate(stringifyCell(job.agentId), CRON_AGENT_PAD), CRON_AGENT_PAD);
    const modelLabel = pad(
      truncate(
        stringifyCell(job.payload?.kind === "agentTurn" ? job.payload.model : undefined),
        CRON_MODEL_PAD,
      ),
      CRON_MODEL_PAD,
    );
