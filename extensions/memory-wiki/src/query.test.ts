...
describe("searchMemoryWiki", () => {
  ...
  it("includes memory results and backfills wiki capacity for all-corpus search", async () => {
    // Wiki BM25 emits integer scores while memory emits 0-1 similarity, so a raw
    // merge-and-slice would drop every memory hit once there are >= maxResults
    // wiki hits.
    const { rootDir, config } = await createQueryVault({
      initialize: true,
      config: {
    ...
    expect(results.some((result) => result.corpus === "memory")).toBe(true);
    // The single memory hit takes one slot; the other four are backfilled with
    // the top wiki hits rather than being lost to the raw score sort.
    expect(
