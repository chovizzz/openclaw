...
describe("sendMediaFeishu msg_type routing", () => {
  ...
  it("suppresses reply text only for native voice media", () => {
    // .ogg / .opus are delivered as native voice bubbles, which carry their own transcription.
    expect(
      shouldSuppressFeishuTextForVoiceMedia({
    ...
    expect(
      shouldSuppressFeishuTextForVoiceMedia({
        mediaUrl: "https://example.com/clip",
    ...
    // Plain audio files are sent as file attachments, so their text must still be delivered.
    expect(
      shouldSuppressFeishuTextForVoiceMedia({
        mediaUrl: "https://example.com/song.mp3",
      }),
    ).toBe(false);
    expect(shouldSuppressFeishuTextForVoiceMedia({})).toBe(false);
  });
  ...
  it("falls back to top-level file sends for thrown withdrawn reply errors", async () => {
    ...
    expect(messageCreateMock.mock.calls[0][0].data.msg_type).toBe("media");
  });
